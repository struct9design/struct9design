  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-humo">Leads</h1>
          <p className="text-xs text-humo/40 mt-1">Solicitudes entrantes del formulario web</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-white/5 bg-pizarra/10 overflow-hidden">
        {loading ? (
          <div className="py-12 text-center text-sm text-humo/30">Cargando leads...</div>
        ) : filtered.length === 0 ? (
          <div className="py-16 text-center">
            <Inbox className="h-8 w-8 text-humo/15 mx-auto mb-3" />
                  ...
                  <Fragment key={lead.id}>
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="font-medium text-humo">{lead.name}</div>
                        <a
                          href={`mailto:${lead.email}`}
                          className="text-xs text-humo/40 hover:text-oro transition-colors"
                        >
                          {lead.email}
                        </a>
                        {lead.phone && (
                          <div className="text-xs text-humo/30 mt-0.5">{lead.phone}</div>
                        )}
                      </td>

                      {/* Service */}
                      <td className="px-5 py-3 text-humo/60 text-xs">{lead.service ?? '—'}</td>
                        ...
                        <div className="flex items-center gap-2">
                          {lead.message && (
                            <button
                              ...
                              className="text-humo/25 hover:text-humo/70 transition-colors"
                              title="Ver mensaje"
                            >
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 text-[10px] font-semibold text-oro/60 hover:text-oro transition-colors whitespace-nowrap"
                    ...
                    {isExpanded && lead.message && (
                      <tr className="border-b border-white/5 bg-white/[0.015]">
                        <td colSpan={5} className="px-5 py-3">
                          <p className="text-xs text-humo/50 leading-relaxed whitespace-pre-wrap">{lead.message}</p>
                        </td>
                      </tr>
                    )}
                  </Fragment>
