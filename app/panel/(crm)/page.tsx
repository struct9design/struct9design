...
import { Plus, TrendingUp, FolderKanban, Users, AlertCircle, Inbox, ArrowRight, Clock } from 'lucide-react'
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-humo">Dashboard</h1>
          <p className="text-sm text-humo/40 mt-1">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <Link
            key={label}
            href={href}
            className="rounded-xl border border-white/5 bg-pizarra/20 p-6 hover:bg-pizarra/30 transition-colors group"
          >
            <div className="flex items-center gap-3 mb-3">
              <Icon className={`h-4 w-4 ${color}`} />
              <span className="text-xs text-humo/50">{label}</span>
            </div>
                ...
                {recentLeads.map(l => {
                  ...
                  return (
                    <tr key={l.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-5 py-3 text-humo/50 text-xs">{l.service ?? '—'}</td>
                      <td className="px-5 py-3">
