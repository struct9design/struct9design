...
export default function NewProjectModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl border border-white/10 bg-tinta shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 md:px-6 py-4 border-b border-white/5 bg-tinta">
          <div>
            <h2 className="text-base md:text-lg font-bold text-humo">Convertir lead a proyecto</h2>
            <p className="text-xs text-humo/40 mt-0.5">{lead.name} · {lead.email}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 md:p-6 space-y-4">
          ...
          {form.kit && (
            <div className="rounded-lg bg-oro/5 border border-oro/20 px-3 py-2 text-xs text-oro">
          ...
          <div>
            <label className="block text-xs font-medium text-humo/60 mb-1.5">Estado inicial</label>
            <select
              value={form.status}
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 rounded-lg bg-oro px-4 py-2.5 text-sm font-bold text-grafito hover:bg-oro/80 disabled:opacity-60 transition-colors"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg border border-white/10 px-4 py-2.5 text-sm text-humo/60 hover:text-humo transition-colors"
            >
              Cancelar
            </button>
          </div>
